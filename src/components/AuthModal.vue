<template>
  <div>
    <div
      @click="$emit('close')"
      class="w-full fixed bg-black/60 top-0 left-0 h-screen"
    ></div>
    <div
      class="fixed md:w-[500px] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white p-7 rounded"
    >
      <div
        class="text-[16px] flex justify-end cursor-pointer"
        @click="$emit('close')"
      >
        x
      </div>
      <div class="flex divide-x-2 divide-green-400 justify-center">
        <button
          @click="content = 'login'"
          class="py-1 px-5 text-[22px] text-[#3D3D3D]"
          :class="content == 'login' ? 'text-green-400' : ''"
        >
          Login
        </button>
        <button
          ...
          class="py-1 px-5 text-[22px] text-[#3D3D3D]"
          :class="content == 'register' ? 'text-green-400' : ''"
        >
          Register
        </button>
      </div>
      <div v-if="content === 'login'" class="w-[360px] mx-auto">
        <p class="text-[#3D3D3D] text-center p-5">
          Enter your username and password to login.
        </p>
        <div class="p-5">
          <input
            class="text-[#A5A5A5] p-4 border rounded pr-28"
            type="email"
        ...
        <p class="text-center text-[#3D3D3D] m-2">Or login with</p>
        <div class="">
          <button
            class="flex gap-2 items-center py-4 px-[80px] text-center border rounded m-5"
          >
            <svg
              width="20"
              height="20"
        ...
        <button
          class="flex gap-2 py-4 px-[72px] text-center border rounded m-5 items-center"
        >
          <svg
            width="20"
            height="20"
      ...
      <div v-if="content === 'register'" class="w-[360px] mx-auto">
        <p class="text-[#3D3D3D] text-center p-5">
          Enter your username and password to login.
        </p>
        <div class="px-5 py-2">
        ...
        <p class="text-center text-[#3D3D3D] m-2">Or register with</p>
        <div class="">
          <button
            class="flex gap-2 items-center py-4 px-[80px] text-center border rounded m-5"
          >
        ...
        <button
          class="flex gap-2 py-4 px-[72px] text-center border rounded mt-5 mb-1 mx-5 items-center"
        >
          <svg
            width="20"
            height="20"
